    ...
    <div className="w-full h-fit flex flex-col justify-start items-start gap-8 py-10 px-6">
      <div className='w-full h-fit flex flex-col gap-2'>
        <div className='w-full h-fit flex flex-row flex-wrap gap-1'>
          <span className='text-xs not-italic font-normal leading-[normal] text-[#b3b3b3] cursor-pointer' onClick={() => gotoPage('/news')}>News & promotions</span>
          <img className='h-4 w-auto' src="/icon/ic-breack.svg" alt="" />
          <span className='text-xs not-italic font-normal leading-[normal] text-[#333]'>New detail</span>
        </div>
        <div className="flex flex-row w-fit h-fit justify-center items-center gap-8 pl-3 border-l-[8px] border-[#0596A6] border-solid">
          <h1 className="text-lg not-italic font-semibold leading-[normal] text-[#1A1A1A]">NEWS & PROMOTIONS</h1>
        </div>
      </div>
      <div className='w-full h-fit flex flex-col justify-center items-center gap-[10px]'>
        <span className='text-base not-italic font-normal leading-[normal] text-[#666]'>{newsDetail?.publish_date ? (new Date(newsDetail?.publish_date)).toString().split(" ").splice(1, 3).join(" ") : ""}</span>
      ...
      <div className='text-base not-italic font-normal leading-[normal] text-[#666] px-10'>
        {parse(newsDetail?.NewsLanguage?.content ?? "")}
      </div>
    </div>
  )
}
