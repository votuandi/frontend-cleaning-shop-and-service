...
import useStyles from './CouponDetail.style'
...
export default function CouponDetail() {
  ...
  const [couponDetail, setCouponDetail] = useState<IGetCouponDetailResponse>()
  ...
  let getCouponDetail = async (_couponId?: number | string) => {
    ...
    try {
      ...
      let res = await couponApi.getDetail({
        params: {
        ...
        <div className={`w-full h-fit flex flex-col gap-[46px] ${isSmallScreen ? 'px-0 py-0' : 'px-6 py-10'}`}>
          ...
          <div className="w-full h-fit flex flex-col gap-6">
            ...
            <div className="flex flex-row flex-wrap justify-start items-center gap-2">
              ...
              <span className="text-[#1a1a1a] text-base font-medium">{couponDetail?.redeem_end_date.split(' ')[0].replaceAll('-', '/')}</span>
            </div>
            {couponDetail?.msg_point && couponDetail.msg_point.length > 0 && (
              <div className="w-full h-fit px-6 py-5 rounded-lg bg-[#EFF7FA] text-center text-[0#0596A6] font-normal">{couponDetail?.msg_point}</div>
            )}
          </div>
              ...
              <div className="w-full h-fit flex flex-col justify-center items-center gap-6">
                ...
                <div className="w-full h-fit flex flex-col justify-center items-center gap-3">
                  ...
                  <TextField
                    ...
                    InputProps={{
                      startAdornment: (
                        <IconButton onClick={() => setAmountRedeem((x) => (x === 0 ? 0 : x - 1))}>
                          <RemoveIcon />
                        </IconButton>
                      ),
                      endAdornment: (
                ...
                <button
                  className={`w-fit h-fit px-12 mx-auto text-center py-[10px] rounded-lg text-base not-italic font-medium leading-[normal] text-white ${
                    amountRedeem === 0 ? 'bg-[#a9a5a5]' : 'bg-[#0596A6]'
                  }`}
                  onClick={handleConfirmRedeem}
                  disabled={amountRedeem === 0}
                >
                  Confirm
                </button>
              </div>
